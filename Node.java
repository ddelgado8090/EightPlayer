...
public class Node{
    ...
    {  
        ...
        //For all of the cases down below, we considered the different
        //positions the blank space may possibly take. We moved the blank
        //space, using the methods we were given(ie. moveBlankDown,
        //moveBlankRight, etc.).
                ...
                if(board[i][j] != goal[i][j]){
                    for (int y = 0; y < size; y++) {
                        for (int z = 0; z < size; z++) {
                            if (board[i][j] == goal[y][z]){
                                correctIndex[0] = y;
                                correctIndex[1] = z;
                            }
                        }
                    } 
                    //traverse through the goal board and then check if the tile at goal board is the same as the one input board
                    //if the location is correct, set i and j to be that index
                    hnvalue += getManhattanDistance(i, j, correctIndex[0], correctIndex[1]);
                }
            }
        }
        return hnvalue;
    ...
    public double cornerHeuristic(){
        double priorityCorner = 0.0;
        for (int i = 0; i < size; i++){
            for (int j = 0; j < size; j++){
                if((board[i][j] == 1 || board[i][j] == 3 || board[i][j] == 7 || board[i][j] == 0) && ((i == 1 && j == 0)||(i == 0 && j == 1) || (i == 1 && j == 1) || (i == 2 && j == 1) || (i == 1 && j == 2))){
                    priorityCorner++;
                }
            }
        }
